...
import numpy as np
from assistive_gym.ppo_new.Anticipation.lib.model.layers import EncoderLayer, DecoderLayer
import torch.nn.functional as F
import assistive_gym.ppo_new.Anticipation.lib.utils.utils as utils
...
class Transformer(nn.Module):
    ...
    def __init__(
            self, input_dim = 7, src_pad_idx=1, trg_pad_idx=1,
        ...
        self.proj=nn.Linear(input_dim,d_model) # 7: 7joints
        self.proj2=nn.Linear(input_dim,d_model)
        ...
        self.l2s = nn.ModuleList([
            nn.Linear(d_model*4, d_model*10)
            for i in range(depth)])
        
        self.projs = nn.ModuleList([
            nn.Linear(d_model, input_dim) # modified
            for i in range(depth)])
    ...
    def forward(self, src_seq, trg_seq, input_seq, use=None):
        ...
        others=input_seq[:,:,:,:].view(bs,-1,self.input_dim)
        ...
        for i in range(self.k_levels+1):
            ...
            dec_output = dec_output.reshape(dec_output.shape[0],10,self.d_model)
