...
class DrinkingEnv(AssistiveEnv):
    ...
    def step(self, action, args, estimated_weights, success_rate):
        ...
        # prefv, preff, prefh, prefhit, preffv = self.human_preferences(end_effector_velocity=end_effector_velocity, total_force_on_human=self.total_force_on_human, tool_force_at_target=self.cup_force_on_human, food_hit_human_reward=water_hit_human_reward, food_mouth_velocities=water_mouth_velocities)
        ...
        human_food_reward = self.config('drinking_reward_weight')*reward_water
        human_tilt_reward = self.config('cup_tilt_weight')*reward_tilt
        ...
        robot_food_reward =  self.config('drinking_reward_weight')*reward_water
        # robot_pref_reward =  preferences_score
        robot_tilt_reward =  self.config('cup_tilt_weight')*reward_tilt # 这里比较特殊，是推动robot去倾斜杯子
        if args.algo == 'PPO':
            if args.social:
                if args.dempref:
        ...
        reward_human = human_dist_reward + human_action_reward + human_tilt_reward + human_food_reward + human_pref_reward
        reward_robot = robot_dist_reward + robot_action_reward + robot_tilt_reward + robot_food_reward + robot_pref_reward
        ...
        info = {'particles' : self.task_success, 'total_force_on_human': self.total_force_on_human, 'task_success': int(self.task_success >= self.total_water_count*self.config('task_success_threshold')), 'action_robot_len': self.action_robot_len, 'action_human_len': self.action_human_len, 'obs_robot_len': self.obs_robot_len, 'obs_human_len': self.obs_human_len}
        ...
        if not self.human.controllable:
            ...
            # Co-optimization with both human and robot controllable 改动
