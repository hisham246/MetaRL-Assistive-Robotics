...
class DressingEnv(AssistiveEnv):
    ...
    def human_preferences(self, end_effector_velocity=0, total_force_on_human=0, tool_force_at_target=0, food_hit_human_reward=0, food_mouth_velocities=[], dressing_forces=[[]], arm_manipulation_tool_forces_on_human=[0, 0], arm_manipulation_total_force_on_human=0):
        ...
        return self.C_v*reward_velocity, self.C_d*reward_dressing_force, self.C_hf*reward_high_target_forces, self.C_fd*reward_food_hit_human, self.C_fdv*reward_food_velocities
    ...
    def step(self, action, args):
        ...
        # dressing force是第二项
        prefv, preff, prefh, prefhit, preffv = self.human_preferences(end_effector_velocity=end_effector_velocity, dressing_forces=self.cloth_forces)
        ...
        human_dist_reward =  self.config('distance_weight')*0
        human_action_reward = self.config('action_weight')*reward_action
        human_food_reward = self.config('dressing_reward_weight')*reward_dressing # 记得food reward就 dressing reward 
        human_pref_reward = preferences_score
        robot_dist_reward =  self.config('distance_weight')*0
        robot_action_reward = self.config('action_weight')*reward_action
        robot_food_reward =  self.config('dressing_reward_weight')*reward_dressing #
        robot_pref_reward =  0
